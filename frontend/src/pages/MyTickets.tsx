    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Tracks every purchase and its payment status.</p>
        </div>
      </div>
            ...
            <div key={t.id} className="p-4 flex flex-wrap items-center gap-4">
              <div className="flex-1 min-w-[200px]">
                ...
                <div className="text-sm text-slate-500">
                  ...
                  {t.event?.startAt && format(new Date(t.event.startAt), 'PPP p')}
                </div>
                <div className="text-xs text-slate-500 mt-1">Order {t.orderRef}</div>
              </div>
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full border font-medium ${statusStyle[t.status]}`}
              >
                {t.status}
              </span>
              {t.status === 'PENDING' && (
